...
function eg_mpc_quad(model::EntryVehicle,A,B,X,U,ϵ_f)
    ...
    for i = 1:N
        ...
        A_ineq[i,idx_x[i][15]] = 1
        A_ineq[i,idx_x[i][23]] = 1
    end
    ...
    P[idx_x[N][15],idx_x[N][15]] = γ
    q[idx_x[N][15]] = γ*(X[N][15] - ϵ_f)
    P[idx_x[N][23],idx_x[N][23]] = γ
    q[idx_x[N][23]] = γ*(X[N][23] - ϵ_f)
    # rr = normalize(xf[1:3])
    # Qn = 1000*(I - rr*rr')
...
function eg_mpc_pdip(model::EntryVehicle,A,B,X,U,ϵ_f)
    ...
    nc = (N-1)*nx + nx #+ (N)*1
    A_eq = spzeros(nc,nz)
    A_ineq = spzeros(N*3,nz) # state constraints

    # dynamics constraint (equality)
    idx_c = [(i-1)*(nx) .+ (1:nx) for i = 1:(N-1)]
    for i = 1:(N-1)
        A_eq[idx_c[i],idx_x[i]]   = sparse(A[i])
        A_eq[idx_c[i],idx_u[i]]   = sparse(B[i])
        A_eq[idx_c[i],idx_x[i+1]] = -I(nx)
    end
    A_eq[(N-1)*nx .+ (1:nx), idx_x[1]] = I(nx)
    ...
    for i = 1:N
        A_ineq[i+N,idx_x[i][15]] = 1
    end
    for i = 1:N
        A_ineq[i+2*N,idx_x[i][23]] = 1
    end
    ...
    P[idx_x[N][8],idx_x[N][8]] = γ
    q[idx_x[N][8]] = γ*(X[N][8] - ϵ_f)
    P[idx_x[N][16],idx_x[N][16]] = γ
    q[idx_x[N][16]] = γ*(X[N][16] - ϵ_f)
    P[idx_x[N][24],idx_x[N][24]] = γ
    q[idx_x[N][24]] = γ*(X[N][24] - ϵ_f)
    # rr = normalize(xf[1:3])
    # Qn = 1000*(I - rr*rr')
    # P[idx_x[N][1:3],idx_x[N][1:3]] = Qn'*Qn
    # q[idx_x[N][1:3]] = -(Qn'*Qn)'*(xf[1:3] - X[N][1:3])
